...
public:
    ...
    static address   bcp(JavaThread *thread)           { return last_frame(thread).interpreter_frame_bcp(); }
    static int       bci(JavaThread *thread)           { return last_frame(thread).interpreter_frame_bci(); }
    static void      set_bcp_and_mdp(address bcp, JavaThread*thread){ last_frame(thread).interpreter_frame_set_bcp(bcp); }
    static Bytecodes::Code code(JavaThread *thread)    {
        // pass method to avoid calling unsafe bcp_to_method (partial fix 4926272)
        return Bytecodes::code_at(method(thread), bcp(thread));
    }
    ...
    static ConstantPoolCacheEntry* cache_entry(JavaThread *thread)            { return cache_entry_at(thread, Bytes::get_native_u2(bcp(thread) + 1)); }
    static void      note_trap_inner(JavaThread* thread, int reason,
                                     methodHandle trap_method, int trap_bci, TRAPS){ CALL_NULL; }
    static void      note_trap(JavaThread *thread, int reason, TRAPS){ CALL_NULL; }
#ifdef CC_INTERP
    // Profile traps in C++ interpreter.
  static void      note_trap(JavaThread* thread, int reason, Method *method, int trap_bci);
#endif // CC_INTERP
    ...
    // Inner work method for Interpreter's frequency counter overflow.
    static nmethod* frequency_counter_overflow_inner(JavaThread* thread, address branch_bcp){ CALL_NULL; }

public:
    // Constants
    static void    ldc           (JavaThread* thread, bool wide){ CALL_NULL; }
    static void    resolve_ldc   (JavaThread* thread, Bytecodes::Code bytecode){ CALL_NULL; }

    // Allocation
    static void    _new          (JavaThread* thread, ConstantPool* pool, int index){ CALL_NULL; }
    static void    newarray      (JavaThread* thread, BasicType type, jint size){ CALL_NULL; }
    static void    anewarray     (JavaThread* thread, ConstantPool* pool, int index, jint size){ CALL_NULL; }
    static void    multianewarray(JavaThread* thread, jint* first_size_address){ CALL_NULL; }
    static void    register_finalizer(JavaThread* thread, oopDesc* obj){ CALL_NULL; }

    // Quicken instance-of and check-cast bytecodes
    static void    quicken_io_cc(JavaThread* thread){ CALL_NULL; }

    // Exceptions thrown by the interpreter
    static void    throw_AbstractMethodError(JavaThread* thread);
    static void    throw_IncompatibleClassChangeError(JavaThread* thread){ CALL_NULL; }
    static void    throw_StackOverflowError(JavaThread* thread);
    static void    throw_ArrayIndexOutOfBoundsException(JavaThread* thread, char* name, jint index);
    static void    throw_ClassCastException(JavaThread* thread, oopDesc* obj);
    static void    create_exception(JavaThread* thread, char* name, char* message);
    ...
    static address exception_handler_for_exception(JavaThread* thread, oopDesc* exception);
#if INCLUDE_JVMTI
    static void    member_name_arg_or_null(JavaThread* thread, address dmh, Method* m, address bcp){ CALL_NULL; }
#endif
    static void    throw_pending_exception(JavaThread* thread){ }
  ...
  static void    note_arrayCheck_trap(JavaThread* thread, Method *method, int trap_bci);
  // A dummy for makros that shall not profile traps.
  static void    note_no_trap(JavaThread* thread, Method *method, int trap_bci) { CALL_NULL; }
#endif // CC_INTERP

    // Statics & fields
    static void    resolve_get_put(JavaThread* thread, Bytecodes::Code bytecode){ CALL_NULL; }

    // Synchronization
    static void    monitorenter(JavaThread* thread, BasicObjectLock* elem){ CALL_NULL_THROW }
    static void    monitorexit (JavaThread* thread, BasicObjectLock* elem){ CALL_NULL_THROW }

    static void    throw_illegal_monitor_state_exception(JavaThread* thread){ CALL_NULL; }
    static void    new_illegal_monitor_state_exception(JavaThread* thread){ CALL_NULL; }

    // Calls
    static void    resolve_invoke       (JavaThread* thread, Bytecodes::Code bytecode){ CALL_NULL; }
    static void    resolve_invokehandle (JavaThread* thread){ CALL_NULL; }
    static void    resolve_invokedynamic(JavaThread* thread){ CALL_NULL; }

    // Breakpoints
    static void _breakpoint(JavaThread* thread, Method* method, address bcp){ CALL_NULL; }
    static Bytecodes::Code get_original_bytecode_at(JavaThread* thread, Method* method, address bcp){ CALL_NULL; }
    static void            set_original_bytecode_at(JavaThread* thread, Method* method, address bcp, Bytecodes::Code new_code){ CALL_NULL; }
    static bool is_breakpoint(JavaThread *thread) { return Bytecodes::code_or_bp_at(bcp(thread)) == Bytecodes::_breakpoint; }

    // Safepoints
    static void    at_safepoint(JavaThread* thread){ CALL_NULL; }

    // Debugger support
    static void post_field_access(JavaThread *thread, oopDesc* obj,
                                  ConstantPoolCacheEntry *cp_entry){ CALL_NULL; }
    static void post_field_modification(JavaThread *thread, oopDesc* obj,
                                        ConstantPoolCacheEntry *cp_entry, jvalue *value){ CALL_NULL; }
    static void post_method_entry(JavaThread *thread){ CALL_NULL; }
    static void post_method_exit (JavaThread *thread){ CALL_NULL; }
    static int  interpreter_contains(address pc){ CALL_NULL; }

    // Native signature handlers
    static void prepare_native_call(JavaThread* thread, Method* method){ CALL_NULL; }
    static address slow_signature_handler(JavaThread* thread,
                                          Method* method,
                                          intptr_t* from, intptr_t* to);
...
#if defined(IA32) || defined(AMD64) || defined(ARM)
    // Popframe support (only needed on x86, AMD64 and ARM)
    static void popframe_move_outgoing_args(JavaThread* thread, void* src_address, void* dest_address){ CALL_NULL; }
#endif
...
# include "interpreterRT_ppc.hpp"
    ...
    // Interpreter's frequency counter overflow
    static nmethod* frequency_counter_overflow(JavaThread* thread, address branch_bcp){ CALL_NULL; }

    // Interpreter profiling support
    static jint    bcp_to_di(Method* method, address cur_bcp){ CALL_NULL; }
    static void    profile_method(JavaThread* thread){ CALL_NULL; }
    static void    update_mdp_for_ret(JavaThread* thread, int bci){ CALL_NULL; }
#ifdef ASSERT
    static void    verify_mdp(Method* method, address bcp, address mdp);
#endif // ASSERT
    static MethodCounters* build_method_counters(JavaThread* thread, Method* m){ CALL_NULL; }
};
#endif
