...
address EnclaveMemory::enclave_interpreter_ret = NULL;

address EnclaveMemory::enclave_interpreter_exception_ret = NULL;
...
oop EnclaveMemory::vm_new_string(JavaThread* THREAD, char* utf8_string) {
    ...
    Handle string = java_lang_String::create_from_unicode(chars, length, CHECK_NULL);
    return string();
}
...
void EnclaveMemory::do_copy(oop *obj, std::queue<StarTask> *tasks, std::queue<oop> *clear_tasks) {
    if (!EnclaveGC::is_obj_in_young(address(*obj))) {
...
std::list<void*>* EnclaveMemory::thread_local_mem = new std::list<void*>();
...
void* EnclaveMemory::ermalloc(void* ptr, size_t old_size,size_t size) {
    ...
    free(ptr);
    thread_local_mem->push_back(ptr);
    return r;
}
