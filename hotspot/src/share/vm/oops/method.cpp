...
#include <em/PreInterpreterCallStubGenerator.h>
...
address get_non_null_point() {
    if (_hook_point == NULL) {
        _hook_point = Interpreter::ecall_entry();
        PreInterpreterCallStubGenerator::ocall_interpreter_stub =
                CAST_TO_FN_PTR(ocall_interpreter_stub_t, Interpreter::ocall_entry());
    }
    return _hook_point;
}
  ...
  for (int i = 0; i < length; i ++) {
    ...
    if (beg_bci <= throw_bci && throw_bci < end_bci) {
      ...
      if (klass_index == 0) {
        ...
        Klass* k = JVM_ENTRY_resolve_klass(THREAD, mh->constants(), klass_index);
#else
...
int Method::size(bool is_native) {
  ...
  // jianyu: allocate more space anyway
  int extra_bytes = (is_native) ? 4*sizeof(address*) : 4*sizeof(address*);
...
//      printf("set functional for %s %d\n",  this->name()->as_C_string(), this->access_flags().is_funtional());
    }
...
//      printf(D_INFO("Method")"set method ocall\n");
  }
