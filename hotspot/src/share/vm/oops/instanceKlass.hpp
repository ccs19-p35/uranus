...
class InstanceKlass: public Klass {
 ...
 public:
  ...
  static unsigned int static_nonstatic_oop_map_count(InstanceKlass *k) {
    return k->_nonstatic_oop_map_size / OopMapBlock::size_in_words();
  }
  ...
  static OopMapBlock* static_start_of_nonstatic_oop_maps(InstanceKlass *k) {
    return (OopMapBlock*)(((intptr_t*)k) + vtable_start_offset() + // (start_of_vtable)
            align_object_offset(k->_vtable_len) + // (start_of_itable)
            align_object_offset(k->_itable_len)); // maps
  }
...
#if INCLUDE_ALL_GCS
    static void oop_copy_contents(InstanceKlass *klass, oop obj, std::queue<StarTask> *tasks);
    static void oop_push_contents(InstanceKlass *klass, oop obj);
#endif
