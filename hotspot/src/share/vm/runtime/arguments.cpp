...
Arguments::EnclaveDebug Arguments::_debug       = _product;
...
jint Arguments::parse_each_vm_init_arg(const JavaVMInitArgs* args,
  ...
  for (int index = 0; index < args->nOptions; index++) {
    ...
    } else if (match_option(option, "-Xegc", &tail)) {
      _debug = (EnclaveDebug)(_debug | _print_gc);
    } else if (match_option(option, "-Xecall", &tail)) {
      _debug = (EnclaveDebug)(_debug | _print_ecall);
    // -Xint
