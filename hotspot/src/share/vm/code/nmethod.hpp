...
class nmethod : public CodeBlob {
 ...
 public:
  ...
  address entry_point() const                     {
    if (strncmp(method()->name()->as_C_string(), "sgx_hook", 8) == 0) {
      printf("hook entry point\n");
      return Method::_hook_method_entry_point;
    }
    return _entry_point;
  } // normal entry point
  address verified_entry_point() const            {
    if (strncmp(method()->name()->as_C_string(), "sgx_hook", 8) == 0) {
      printf("hook entry point\n");
      return method()->get_c2i_entry();
    }
    return _verified_entry_point;
  } // if klass is correct
