...
#else
int java_lang_String::value_offset  = 16;
int java_lang_String::offset_offset = 0;
int java_lang_String::count_offset  = 0;
int java_lang_String::hash_offset   = 24;
#endif
...
#else
  obj = (oop) EnclaveMemory::static_vm_new_obj(THREAD, EnclaveMemory::wk_classes[SystemDictionary::String_klass_knum]);
#endif
...
#else
    buffer = (typeArrayOop) EnclaveMemory::static_vm_type_array(THREAD, T_CHAR, length);
#endif
...
void java_lang_Throwable::fill_in_stack_trace(Handle throwable, methodHandle method, TRAPS) {
  ...
  for (frame fr = thread->last_frame(); max_depth != total_count;) {
    ...
    if (decode_offset != 0) {
      ...
      if (fr.is_first_frame() || fr.is_enclave_frame()) break;
      address pc = fr.pc();
...
int java_lang_Class::_klass_offset = 128;
int java_lang_Class::_array_klass_offset = 136;
