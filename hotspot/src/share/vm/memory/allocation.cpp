...
#ifdef ENCLAVE_UNIX
void* emalloc_stub(int size) {
  return EnclaveMemory::emalloc(size);
}
void* ermalloc_stub(void* old, int new_size) {
  return EnclaveMemory::ermalloc(old, new_size, new_size);
}
void efree_stub(void* old) {
...
void* ResourceObj::operator new(size_t size, allocation_type type, MEMFLAGS flags) throw() {
  ...
  return res;
#else
  // TODO: prevent potential mem leak
  return malloc(size);
#endif
}
