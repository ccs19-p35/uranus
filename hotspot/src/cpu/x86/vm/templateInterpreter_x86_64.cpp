...
//    __ movptr(Address(r12, 0), rax);
  ...
  __ set_last_Java_frame(r15, rsp, rbp, __ pc());
  ...
  __ reset_last_Java_frame(r15, true);
...
#ifdef ENCLAVE_UNIX
  __ call_VM(rax, CAST_FROM_FN_PTR(address, InterpreterRuntime::throw_StackOverflowError));
#else
...
address InterpreterGenerator::generate_native_entry(bool synchronized) {
  ...
  __ call_VME(CAST_FROM_FN_PTR(address, print_native_enclave_frame), false, true);
  ...
  // TODO-jianyu: copy those parameter to enclave_stack?
...
#ifdef ENCLAVE_UNIX
   __ cmpl(r11, 0);
   __ jcc(Assembler::equal, par_fin);

   __ movl(r12, 0);
   __ lea(r14, Address(r14, rcx, Address::times_8));
   {
     Label exit, loop;
     __ testl(rcx, rcx);
     __ jcc(Assembler::lessEqual, exit); // do nothing if rcx <= 0
     __ bind(loop);
     __ movptr(r11, Address(r14, 0));
     __ push(r11);
     __ addl(r12, 1);
     __ lea(r14, Address(r14, -1, Address::times_8));
     __ cmpl(r12, rcx);
...
void TemplateInterpreterGenerator::generate_throw_exception() {
  // forward throw into interpreter
  // after that, rax -> exception, rdx throwing pc
  // no need to jump to rethrow
  Interpreter::_throw_forward_entry = __ pc();
  __ pop(rdx);
...
#ifdef DB_EXP
  // should do this after expression is empty
  __ push(rax);
  __ push(rdx);
  __ push(rcx);
  __ push(c_rarg0);
  ...
  __ mov(c_rarg0, r15_thread);
  __ mov(c_rarg1, rax);
  __ call_VME(CAST_FROM_FN_PTR(address, print_exception_frame), false, true);
  ...
  #else
    Label in_interpreter;
    __ movptr(c_rarg0, rdx);
    __ movptr(c_rarg1, r15);
    __ call(RuntimeAddress((address)EnclaveException::check_exit_addr));
    __ mov(rbx, rax);
    __ pop(rdx);                                   // restore return address
    __ pop(rax);
    __ testptr(rbx, rbx);
    __ jcc(Assembler::notZero, in_interpreter);
    __ addptr(rdx, (EnclaveMemory::enclave_interpreter_exception_ret - EnclaveMemory::enclave_interpreter_ret));
    __ jmp(rdx);
    __ bind(in_interpreter);
  #endif
