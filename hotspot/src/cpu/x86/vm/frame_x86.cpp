...
bool frame::is_enclave_frame() const {
    if ((intptr_t)_fp == (intptr_t)(-1) || (intptr_t)_unextended_sp == (intptr_t)-1) {
        printf(D_WARN("Check Frame")" is enclave frame %d\n", JavaThread::current()->is_in_ecall);
    }
    return ((intptr_t)_fp == (intptr_t)(-1) || (intptr_t)_unextended_sp == (intptr_t)(-1));
}
...
frame frame::sender(RegisterMap* map) const {
  ...
  if (is_enclave_frame()) {
      printf("find sender of enclave frame:%lx, fp:%lx, sp:%lx\n", this, this->fp(), this->unextended_sp());
      BREAKPOINT;
      ShouldNotReachHere();
  }
