...
// create the PerfData memory region in standard memory.
//
static char* create_standard_memory(size_t size) { syscall_invoke_count(__func__); }


static void delete_standard_memory(char* addr, size_t size) { syscall_invoke_count(__func__); }

static void save_memory_to_file(char* addr, size_t size) { syscall_invoke_count(__func__); }
...
// the caller is expected to free the allocated memory.
//
static char* get_user_tmp_dir(const char* user) { syscall_invoke_count(__func__); }

// convert the given file name into a process id. if the file
// does not meet the file naming constraints, return 0.
//
static pid_t filename_to_pid(const char* filename) { syscall_invoke_count(__func__); }
...
// if an error occurred.
//
static bool is_statbuf_secure(struct stat *statp) { syscall_invoke_count(__func__); }
...
// is a symbolic link or if an error occurred.
//
static bool is_directory_secure(const char* path) { syscall_invoke_count(__func__); }
...
// is a symbolic link or if an error occurred.
//
static bool is_dirfd_secure(int dir_fd) { syscall_invoke_count(__func__); }


// Check to make sure fd1 and fd2 are referencing the same file system object.
//
static bool is_same_fsobject(int fd1, int fd2) { syscall_invoke_count(__func__); }


// Open the directory of the given path and validate it.
// Return a DIR * of the open directory.
//
static DIR *open_directory_secure(const char* dirname) { syscall_invoke_count(__func__); }
...
// Return a DIR * of the open directory and the saved cwd fd.
//
static DIR *open_directory_secure_cwd(const char* dirname, int *saved_cwd_fd) { syscall_invoke_count(__func__); }

// Close the directory and restore the current working directory.
//
static void close_directory_secure_cwd(DIR* dirp, int saved_cwd_fd) { syscall_invoke_count(__func__); }

// Check if the given file descriptor is considered a secure.
//
static bool is_file_secure(int fd, const char *filename) { syscall_invoke_count(__func__); }
...
// the caller is expected to free the allocated memory.
//
static char* get_user_name(uid_t uid) { syscall_invoke_count(__func__); }
...
// the caller is expected to free the allocated memory.
//
static char* get_user_name_slow(int vmid, TRAPS) { syscall_invoke_count(__func__); }
...
// the caller is expected to free the allocated memory.
//
static char* get_sharedmem_filename(const char* dirname, int vmid) { syscall_invoke_count(__func__); }
...
// this method removes the file specified by the given path
//
static void remove_file(const char* path) { syscall_invoke_count(__func__); }
...
// any stale file resources are removed.
//
static void cleanup_sharedmem_resources(const char* dirname) { syscall_invoke_count(__func__); }
...
// insecure, or if an error occurred.
//
static bool make_user_tmp_dir(const char* dirname) { syscall_invoke_count(__func__); }
...
// it does not yet exist.
//
static int create_sharedmem_resources(const char* dirname, const char* filename, size_t size) { syscall_invoke_count(__func__); }

// open the shared memory file for the given user and vmid. returns
// the file descriptor for the open file or -1 if the file could not
// be opened.
//
static int open_sharedmem_file(const char* filename, int oflags, TRAPS) { syscall_invoke_count(__func__); }
...
// for files being created or removed.
//
static char* mmap_create_shared(size_t size) { syscall_invoke_count(__func__); }

// release a named shared memory region
//
static void unmap_shared(char* addr, size_t bytes) { syscall_invoke_count(__func__); }

// create the PerfData memory region in shared memory.
//
static char* create_shared_memory(size_t size) { syscall_invoke_count(__func__); }

// delete the shared PerfData memory region
//
static void delete_shared_memory(char* addr, size_t size) { syscall_invoke_count(__func__); }

// return the size of the file for the given file descriptor
// or 0 if it is not a valid size for a shared memory file
//
static size_t sharedmem_filesize(int fd, TRAPS) { syscall_invoke_count(__func__); }

// attach to a named shared memory region.
//
static void mmap_attach_shared(const char* user, int vmid, PerfMemory::PerfMemoryMode mode, char** addr, size_t* sizep, TRAPS) { syscall_invoke_count(__func__); }
...
// shared memory.
//
void PerfMemory::create_memory_region(size_t size) { syscall_invoke_count(__func__); }
...
// tuple will be inaccessible after a call to this method.
//
void PerfMemory::delete_memory_region() { syscall_invoke_count(__func__); }
...
// address space.
//
void PerfMemory::attach(const char* user, int vmid, PerfMemoryMode mode, char** addrp, size_t* sizep, TRAPS) { syscall_invoke_count(__func__); }
...
// process's address space.
//
void PerfMemory::detach(char* addr, size_t bytes, TRAPS) { syscall_invoke_count(__func__); }

char* PerfMemory::backing_store_filename() { syscall_invoke_count(__func__); }
