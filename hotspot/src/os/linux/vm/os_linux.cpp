...
void os::jvm_path(char *buf, jint buflen) {
  ...
  CompilerEnclave::enclave_so_path =
          new char[MAXPATHLEN - sizeof(ENCLAVE_SO_FILE) - sizeof("libjvm.so")];
  int n = strlen(saved_jvm_path);
  strcpy(CompilerEnclave::enclave_so_path, saved_jvm_path);
  strncpy(CompilerEnclave::enclave_so_path + n - sizeof("libjvm.so") + 1,
          ENCLAVE_SO_FILE, sizeof(ENCLAVE_SO_FILE));
}
...
CompilerEnclave *cv = NULL;

extern "C" JNIEXPORT int
JVM_handle_linux_signal(int signo, siginfo_t* siginfo,
                        void* ucontext, int abort_if_unrecognized);

int enclaveSignalHandler(int sig, siginfo_t* info, void* uc) {
    ucontext_t* context = (ucontext_t*) uc;
    address stack_frame = (address)context->uc_stack.ss_sp;
    CompilerEnclave *enclave = (cv == NULL)? new CompilerEnclave(): cv;
    if (enclave->in_enclave(stack_frame)) {
        return 1;
    }
    return 0;
}
...
//  if (enclaveSignalHandler(sig, info, uc)) {
//      return;
//  }
...
void Parker::park(bool isAbsolute, jlong time) { syscall_invoke_count(__func__); }

int os::loadavg(double loadavg[], int nelem) { syscall_invoke_count(__func__); }

void Parker::unpark() { syscall_invoke_count(__func__); }
...
void os::javaTimeNanos_info(jvmtiTimerInfo *info_ptr) { syscall_invoke_count(__func__); }

void os::current_thread_cpu_time_info(jvmtiTimerInfo *info_ptr) { syscall_invoke_count(__func__); }
...
void os::thread_cpu_time_info(jvmtiTimerInfo *info_ptr) { syscall_invoke_count(__func__); }
...
void os::thread_local_storage_at_put(int index, void* value) { syscall_invoke_count(__func__); }
