...
void MacroAssembler::warn(const char* msg) {
  ...
  lea(c_rarg0, ExternalAddress((address) msg));
  call_VM_leaf(CAST_FROM_FN_PTR(address, printf), c_rarg0);
  pop_CPU_state();
  mov(rsp, rbp);
  pop(rbp);
}
...
void MacroAssembler::call_VME(address entry_point, bool check_exceptions, bool set_frame) {
...
//      movptr(Address(rbp, frame::interpreter_frame_bcx_offset * wordSize), r13);
      ...
      set_last_Java_frame(r15_thread, last_java_sp, rbp, NULL);
  }
  call(RuntimeAddress(entry_point));
  if (set_frame) {
      reset_last_Java_frame(r15_thread, true);
      // quick fix to avoid gc in runtime
//      movptr(r13, Address(rbp, frame::interpreter_frame_bcx_offset * wordSize));
  ...
  if (check_exceptions) {
    ...
    // r15 is callee-saved
    cmpptr(Address(r15_thread, Thread::pending_exception_offset()), (int32_t) NULL_WORD);

    Label ok;
    jcc(Assembler::equal, ok);
    jump(RuntimeAddress(Interpreter::throw_forward_entry()));
    bind(ok);
  }
}
...
void MacroAssembler::eden_allocate(Register obj,
                                   ...
                                   Label& slow_case) {
  Register end = t1;
  Register top = rscratch1;
  Register bottom = rscratch2;
  Label retry;
  bind(retry);
  Address heap_top(r15_thread, JavaThread::mem_offset() + EnclaveGC::eden_start_offset());
  Address heap_bottom(r15_thread, JavaThread::mem_offset() + EnclaveGC::eden_end_offset());

  lea(top, heap_top);
  lea(bottom, heap_bottom);

  movptr(obj, Address(top, 0));
...
void MacroAssembler::lookup_interface_method(Register recv_klass,
  ...
  lea(recv_klass, Address(recv_klass, itable_index, Address::times_ptr, itentry_off)); //problem
