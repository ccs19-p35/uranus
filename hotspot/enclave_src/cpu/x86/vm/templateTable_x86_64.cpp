...
void TemplateTable::aastore() {
  ...
  do_oop_store(_masm, Address(rdx, 0), rax, BarrierSet::Other, true);
  __ jmp(done);
...
void TemplateTable::putfield_or_static(int byte_no, bool is_static) {
  ...
  // atos
  {
    ...
    do_oop_store(_masm, field, rax, BarrierSet::Other, false);
    if (!is_static) {
...
void TemplateTable::_new() {
  ...
  __ cmpb(Address(rax, rdx, Address::times_1, tags_offset),
          JVM_CONSTANT_Class);
  __ jcc(Assembler::notEqual, do_resolve);
  ...
  if (allow_shared_alloc) {
    ...
    Address top(r15_thread, JavaThread::mem_offset() + EnclaveGC::eden_start_offset());
    Address end(r15_thread, JavaThread::mem_offset() + EnclaveGC::eden_end_offset());
...
void TemplateTable::newarray() {
  ...
  __ movl(c_rarg2, rax);
  __ call_VME(CAST_FROM_FN_PTR(address, EnclaveMemory::static_vm_type_array));
}
...
void TemplateTable::anewarray() {
  ...
  __ get_constant_pool(c_rarg1);
  __ movl(c_rarg3, rax);
  __ call_VME(CAST_FROM_FN_PTR(address, EnclaveMemory::static_cpoll_new_array));
}
...
//    __ load_klass(rdx, rax);
  ...
  // jianyu: rdx store klass of tos obj, rax store klass of instanceof class
  __ bind(resolved);
...
void TemplateTable::multianewarray() {
  transition(vtos, atos);
  // jianyu:
  __ load_unsigned_byte(rax, at_bcp(3));
  __ lea(rcx, Address(rsp, rax, Address::times_8, -wordSize));

  __ get_cpool_and_tags(c_rarg1, rdx);
  __ get_unsigned_2_byte_index_at_bcp(c_rarg2, 1);
  __ mov(c_rarg4, rax);
  __ mov(c_rarg3, rcx);
