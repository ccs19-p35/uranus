...
inline void*     oopDesc::field_base(oopDesc *oop, int offset) {return (void*)&((char*)oop)[offset];}

template <class T> inline T* oopDesc::obj_field_addr(int offset) const { return (T*)field_base(offset); }
template <class T> inline T* oopDesc::obj_field_addr(oopDesc *oop, int offset) { return (T*)oopDesc::field_base(oop, offset); }
...
inline bool oopDesc::is_oop(bool ignore_mark_word) const {
  return CUR_GC->is_obj_in_young((address)this);
//  oop obj = (oop) this;
