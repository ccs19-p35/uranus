...
#define STATIC_InstanceKlass_OOP_MAP_REVERSE_ITERATE(klass, obj, do_oop, assert_fn)    \
{                                                                        \
  OopMapBlock* const start_map = static_start_of_nonstatic_oop_maps(klass);          \
  OopMapBlock* map             = start_map + static_nonstatic_oop_map_count(klass);  \
  if (UseCompressedOops) {                                               \
    while (start_map < map) {                                            \
      --map;                                                             \
      InstanceKlass_SPECIALIZED_OOP_REVERSE_ITERATE(narrowOop,           \
        oopDesc::obj_field_addr<narrowOop>(obj, map->_offset), map->_count,     \
        do_oop, assert_fn)                                               \
    }                                                                    \
  } else {                                                               \
    while (start_map < map) {                                            \
      ...
      InstanceKlass_SPECIALIZED_OOP_REVERSE_ITERATE(oop,                 \
        oopDesc::obj_field_addr<oop>(obj, map->_offset), map->_count,           \
        do_oop, assert_fn)                                               \
    }                                                                    \
  }                                                                      \
}
#define InstanceKlass_OOP_MAP_REVERSE_ITERATE(obj, do_oop, assert_fn) STATIC_InstanceKlass_OOP_MAP_REVERSE_ITERATE(this, obj,do_oop, assert_fn)
...
void InstanceKlass::oop_push_contents(InstanceKlass* klass, oop obj) {
    STATIC_InstanceKlass_OOP_MAP_REVERSE_ITERATE( \
    klass, obj, \
    if (CUR_GC->is_obj_in_young((address)(*p))) { \
      CUR_GC->claim_or_forward_depth(p); \
    }, \
    assert_nothing )
}

void InstanceKlass::oop_adjust_pointers(InstanceKlass* klass, oop obj) {
  STATIC_InstanceKlass_OOP_MAP_REVERSE_ITERATE( \
    klass, obj, \
    if (CUR_GC->is_obj_in_young((address)(*p))) { \
      CUR_GC->adjust_pointers(p); \
    }, \
    assert_nothing )
}

void InstanceKlass::oop_copy_contents(InstanceKlass* klass, oop obj, std::queue<StarTask> *tasks) {
    STATIC_InstanceKlass_OOP_MAP_REVERSE_ITERATE( \
    klass, obj, \
    if (NULL != (*p)) { \
