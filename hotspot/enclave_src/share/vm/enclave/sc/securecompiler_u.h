...
void SGX_UBRIDGE(SGX_NOCONVENTION, ocall_jvm_resolve_get_put, (int byte, void* mh, int idx, void* bcp));
...
int SGX_UBRIDGE(SGX_NOCONVENTION, ftruncate64, (int fd, __off64_t length));
...
sgx_status_t securecompiler_c1_initialize(sgx_enclave_id_t eid, void** retval, void* cpuid, void** top, void** bottom, void** klass_list);
sgx_status_t securecompiler_gc_scavenge(sgx_enclave_id_t eid, void* tasks, int n);
sgx_status_t securecompiler_within_enclave(sgx_enclave_id_t eid, int* retval, void* addr);
sgx_status_t securecompiler_interpreted_entry_zero_locals(sgx_enclave_id_t eid, void** retval, void* rbx, int* exp, int functional);
sgx_status_t securecompiler_ecall_execute_job(sgx_enclave_id_t eid, pthread_t pthread_self_id, unsigned long int job_id);
sgx_status_t securecompiler_ecall_set_enclave_id(sgx_enclave_id_t eid, sgx_enclave_id_t self_eid);
