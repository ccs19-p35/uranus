...
//    memset((obj) + hs, 0,(size - hs) * sizeof(char*));
    ...
    ((oop)obj)->set_mark(CUR_GC->mark_prototype());
    return (oop)obj;
}
...
address EnclaveMemory::allocate(int size, JavaThread* THREAD) {
    address ret = (address)CUR_GC->allocate_eden(size);
    if (ret == NULL) {
        int count = 0;
        while (count < ENCLAVE_ALLOCATION_THREADHOLD && ret == NULL) {
            CUR_GC->enclave_gc(THREAD);
            ret = (address)CUR_GC->allocate_eden(size);
            count += 1;
        }
    }
    if (ret == NULL) {
...
void EnclaveMemory::do_copy(oop *obj, std::queue<StarTask> *tasks, std::queue<oop> *clear_tasks) {
    if (!CUR_GC->is_obj_in_young(address(*obj)) && !within_enclave(*obj)) {
