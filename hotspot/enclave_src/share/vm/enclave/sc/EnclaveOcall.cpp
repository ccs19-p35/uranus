...
void* JVM_ENTRY_resolve_invoke_C(void* recv, void* constant, int bidx, int bc OCallCallerArgs) {
    ...
    DO_OCALL_CHECK(ocall_jvm_resolve_invoke_C(&ret, recv, constant, bidx, bc, recv_klass));
    return ret;
}

void* JVM_ENTRY_resolve_method_C(int bc, void* constant, int idx OCallCallerArgs) {
    char* ret;
    DO_OCALL_CHECK(ocall_jvm_resolve_method_C(&ret, bc, constant, idx));
    return ret;
}
...
void* JVM_ENTRY_ldc(void* thread, bool wide, void *p, int index OCallCallerArgs) {
    ...
    DO_OCALL_CHECK(ocall_jvm_ldc(&ret, wide, p, index));
    return ret;
}
...
//            return result;
//    }
    DO_OCALL_CHECK(ocall_jvm_resolve_ldc(&ret, p, index, bytecode));
    return ret;
}

void* KLASS_get_type_klass(void* dummy OCallCallerArgs) {
    char *ret = NULL;
    DO_OCALL_CHECK(ocall_klass_type(&ret));
    return ret;
}

void* KLASS_get_type_array_klass(void* dummy OCallCallerArgs) {
    char *ret = NULL;
    DO_OCALL_CHECK(ocall_klass_type_array(&ret));
    return ret;
}
...
void* KLASS_get_obj_array_klass(void* pool, int index OCallCallerArgs) {
    ...
    if (super == NULL) {
        DO_OCALL_CHECK(ocall_obj_array_klass_get(&ret, pool, index));
        return ret;
    }
    return super;
}
...
void* KLASS_get_multi_array_klass(void* pool, int index OCallCallerArgs) {
    ...
    DO_OCALL_CHECK(ocall_multi_array_klass_get(&ret, pool, index));
    return ret;
}

void* KLASS_resolve_or_fail(const char* name OCallCallerArgs) {
  char* ret = NULL;
  DO_OCALL_CHECK(ocall_klass_resolve_or_fail(&ret, name));
  Klass* k = (Klass*)ret;
  if (ret != NULL) {
...
void* KLASS_find_or_null(const char* name OCallCallerArgs) {
    ...
    // TODO
    return NULL;
    DO_OCALL_CHECK(ocall_klass_find_or_null(&ret, name));
    Klass* k = (Klass*)ret;
    return ret;
}
...
void* JVM_ENTRY_resolve_klass(void* thread, void* pool, int index OCallCallerArgs) {
    char* ret = NULL;
    DO_OCALL_CHECK(ocall_klass_get(&ret, pool, index));
    return ret;
}
...
void* Klass_resolve_get_put(void* thread, void* method, int index, int bytecode OCallCallerArgs) {
    char* ret;
    DO_OCALL_CHECK(ocall_jvm_resolve_get_put_klass(&ret, bytecode, method, index, 0));
    return ret;
}
...
void* KLASS_array_klass(void* klass, int rank, int get_all OCallCallerArgs) {
    ...
    if (super != NULL)
        return super;
    DO_OCALL_CHECK(ocall_array_klass(&ret, klass, rank, get_all));
    return ret;
}
